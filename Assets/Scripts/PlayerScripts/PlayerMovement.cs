...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Start ( )
    {
        eyesScript = gameObject.GetComponentInChildren<PlayerEyes> ( );
        eyesScript.transform.position = new Vector3 ( transform.position.x, transform.position.y + playerHeight, transform.position.z );
    ...
    public void RotationalMove ( float mouseX, float mouseY )
    {
        ...
        eyesScript.transform.rotation = Quaternion.Euler ( rotationX * IsInversed ( ), transform.rotation.eulerAngles.y, transform.rotation.eulerAngles.z );
